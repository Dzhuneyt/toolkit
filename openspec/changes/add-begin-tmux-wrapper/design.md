...
- **tmux user-option scoping** (`show-option -g` vs per-session) must be verified so `@begin_path` reads back the correct session's value → Pin the exact `set-option`/`show-option -t <session>` flags during implementation and cover with a stubbed-tmux test.
